...
/*
 * The styled bubble draws from `data-tooltip`, not from `title`.
 *
 * `.activation-icon-btn::after` is `content: attr(data-tooltip)`, and the
 * pseudo-element renders whether or not the attribute exists — so a button
 * without it shows an empty dark rectangle on hover. Every icon button in the
 * tab had `title` and `aria-label` and none of them had this, which is exactly
 * as invisible to an assertion about `aria-label` as it was to me.
 */
const bubbles = await reperTable()
  .locator('tbody tr .strategy-row-actions button')
  .evaluateAll((nodes) =>
    nodes.map((node) => (node.getAttribute('data-tooltip') ?? '').trim()),
  );
check('AS-U-02b', 'fiecare buton are data-tooltip cu text',
  bubbles.length > 0 && bubbles.every((text) => text.length > 3),
  bubbles.join(' | '));

// `white-space: nowrap` on the bubble, so a long one runs off the table.
check('AS-U-02c', 'bulele sunt scurte — nu depășesc lățimea rândului',
  bubbles.every((text) => text.length <= 34),
  bubbles.filter((text) => text.length > 34).join(' | ') || 'toate sub 34 de caractere');
